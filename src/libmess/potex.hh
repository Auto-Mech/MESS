...
public:
  ...
  template<typename T>
  void assign (const std::vector<T>&   in, double val)  { assign(_convert(in), val); }
    
  template<typename T>
  void assign (const std::multiset<T>& in, double val)  { assign(_convert(in), val); }
  ...
  template<typename T>
  double operator() (const std::vector<T>&   in) const  { return (*this)(_convert(in)); }

  template<typename T>
  double operator() (const std::multiset<T>& in) const  { return (*this)(_convert(in)); }
...
template<typename T>
double PotentialExpansion::operator() (int r ...) const 
{
...
template<>
inline void PotentialExpansion::assign (const index_t& in, double val) 
{
  _check_index(in);
...
template<>
inline double PotentialExpansion::operator() (const index_t& in) const 
{
  _check_index(in);
