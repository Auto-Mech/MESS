...
namespace Harding {
  ...
  // input parameters
  int                   term_order_max  = 0; // maximal order of individual power terms 
  int                   poly_order_max  = 0; // overall expansion polynomial order maximum
  ...
  int dist_size;                     // dimensionality of the fitting configurational space
  ...
  Array<double> poly_data; // symmetric polynomials values
}

extern "C" void harding_init_ (std::istream& from)
{
  Exception::Base funame = "harding_init: ";

  using namespace Harding;
  ...
  try {
	...
	dist_size = atom_size * (atom_size - 1) / 2;
      }
	...
	try { 
	  while(1)
	    atom_group.insert(Permutation(Permutation::read_orbit(iss), atom_size));
	} 
	catch(Exception::Eof) {}
	...
	for(std::set<Permutation>::const_iterator g = atom_group.begin(); g != atom_group.end(); ++g) {
	  ...
	  std::vector<int> gg(dist_size);
	  for(int i2 = 1; i2 < atom_size; ++i2)
    ...
    if(!symm_group.size())
      symm_group.insert(Permutation(dist_size));

    // setting single term maps
    std::vector<std::vector<int> > term_multi_map;
    term_multi_map.push_back(std::vector<int>(dist_size));

    std::vector<int> term_order_map;
    term_order_map.push_back(0);

    int multi_num = dist_size * term_order_max;
    ...
    for(int b = 0; b < bond_def.size(); ++b) {
      int a0 = bond_def[b][0];
      int a1 = bond_def[b][1];
      if(a0 < 0 || a1 < 0 || a0 >= atom_size || a1 >= atom_size || a0 == a1)
    ...
    poly_data.resize(poly_map.size() + bond_size);
    ...
    IO::log << IO::log_offset << "number of multiplications       = " << multi_num         << "\n"
	    ...
	    << IO::log_offset << "number of distances             = " << dist_size         << "\n"
...
{
  const char funame [] = "update_poly_val: ";
...
{
  Exception::Base funame = "harding_pot: ";
  ...
  try {
    ...
    harding_update_(dist, poly_data);

    dtemp = 0.;

#ifdef OPENMP
#pragma omp parallel for default(shared) reduction(+: dtemp) schedule(static)
#endif

    for(int i = 0; i < poly_data.size(); ++i)
      dtemp += poly_data[i] * coef[i];
