/*
        Chemical Kinetics and Dynamics Library
        Copyright (C) 2008-2013, Yuri Georgievski <ygeorgi@anl.gov>

        This library is free software; you can redistribute it and/or
        modify it under the terms of the GNU Library General Public
        License as published by the Free Software Foundation; either
        version 2 of the License, or (at your option) any later version.

        This library is distributed in the hope that it will be useful,
        but WITHOUT ANY WARRANTY; without even the implied warranty of
        MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
        Library General Public License for more details.
*/

#include "structure.hh"
#include "units.hh"
#include "key.hh"

#include <sstream>
#include <cmath>
#include <cstdio>

double Molecule::tolerance = 1.e-3;

namespace Structure {
  //
  int mute = 0;
  
  Molecule _fragment [2];

  int    _size;
  double _mass;
  double _mass_sqrt;

  int _ang_pos [2];
  int _ang_vel [2];

  int _pos_size;
  int _vel_size;
  int _tot_size;

  int _tm_dof;

  bool _isinit = false;

  // setup
  //
  bool isinit () { return _isinit; }

  void assert ();
}

void Structure::assert () 
{
  if(!isinit()) {
    //
    if(!mute)
      //
    std::cerr << "Structure::assert: not initialized\n";

    throw Error::Init();
  }
}

const Molecule& Structure::fragment (int frag) 
{
  assert();

  return _fragment[frag];
}

int Structure::type (int frag) { return fragment(frag).type(); }
int Structure::top  (int frag) { return fragment(frag).top();  }

int Structure::size    () 
{
  assert();

  return _size;
}

double Structure::mass () 
{ 
  assert();

  return _mass; 
}

double Structure::mass_sqrt () 
{ 
  assert();

  return _mass_sqrt; 
}

int Structure::orb_pos () 
{
  assert();

  return 0;
}

int Structure::ang_pos (int frag) 
{
  assert();

  return _ang_pos[frag];
}

int Structure::orb_vel () 
{
  assert();

  return 0;
}
int Structure::ang_vel (int frag) 
{
  assert();

  return _ang_vel[frag]; 
}

int Structure::pos_size () 
{
  assert();

  return _pos_size;
}
int Structure::vel_size () 
{ 
  assert();

  return _vel_size;
}

int Structure::pos_size (int frag) 
{ 
  return fragment(frag).pos_size(); 
}

int Structure::vel_size (int frag) 
{ 
  return fragment(frag).vel_size(); 
}

int Structure::dv_size () 
{
  assert();

  return _tot_size;
}

int Structure::tm_dof(int frag) 
{
  return fragment(frag).tm_dof();
}

int Structure::tm_dof () 
{
  assert();

  return _tm_dof;
}

void Structure::init (std::istream& from) 
{
  const char funame [] = "Structure::init: ";

  if(_isinit) {
    //
    if(!mute)
      //
      std::cerr << funame << "has been already initialized\n";

    throw Error::Init();
  }

  _isinit = true;


  int    itemp;
  double dtemp;

  std::string token, line, comment;

  KeyGroup StructureGroup;

  Key frag_key("Fragment" );
  Key  tol_key("Tolerance");

  int frag = 0;
  
  while(from >> token) {
    //
    if(token == IO::end_key()) {
      //
      break;
    }
    // fragments
    //
    else if(token == frag_key) {
      //
      if(frag > 1) {
	//
	if(!mute)
	  //
	  std::cerr << funame << "too many fragments\n";
	
	throw Error::Form();
      }
      
      _fragment[frag].read(from);
      
      ++frag;
    }
    // tolerance
    //
    else if(token == tol_key) {
      //
      if(!(from >> Molecule::tolerance)) {
	//
	if(!mute)
	  //
	  std::cerr << funame << token << ": corrupted\n";
	
	throw Error::Input();
      }
      
      if(Molecule::tolerance <= 0. || Molecule::tolerance >= 1.) {
	//
	if(!mute)
	  //
	  std::cerr << funame << token << ": out of range\n";
	
	throw Error::Range();
      }
    }
    // unknown keyword
    //
    else {
      //
      if(!mute) {
	//
	std::cerr << funame << "unknown key: " << token << "\nAvailable keys:\n";
	
	Key::show_all(std::cerr);
	std::cerr << "\n";
      }
      
      throw Error::Form();
    }  
  }
  
  if(!from) {
    //
    if(!mute)
      //
      std::cerr << funame << "input stream is corrupted\n";
    
    throw Error::Form();
  }

  if(frag < 2) {
    //
    if(!mute)
      //
      std::cerr << funame << "fragments were not initialized\n";

    throw Error::Init();
  }
    
  _size = fragment(0).size() + fragment(1).size();
  _mass = fragment(0).mass() * fragment(1).mass() /
    (fragment(0).mass() + fragment(1).mass());
  _mass_sqrt = std::sqrt(_mass);

  _ang_pos[0] = 3;
  _ang_pos[1] = _ang_pos[0] + fragment(0).pos_size();
  _pos_size   = _ang_pos[1] + fragment(1).pos_size();

  _ang_vel[0] = 3;
  _ang_vel[1] = _ang_vel[0] + fragment(0).vel_size();
  _vel_size   = _ang_vel[1] + fragment(1).vel_size();

  _tot_size = _pos_size + _vel_size;

  _tm_dof = 3 + fragment(0).tm_dof() + fragment(1).tm_dof();

  if(!mute)
    //
    for(frag = 0; frag < 2; ++frag)
      //
      IO::log << IO::log_offset << frag << "-th fragment: " << fragment(frag);      
}

double Molecule::imom (int i) const 
{
  const char funame [] = "Molecule::imom: ";

  switch(type()) {
    //
  case MONOATOMIC:
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "should not be used for atomic fragment\n";
    
    throw Error::Logic();
    //
  case LINEAR:
    //
    return _iner_mom[2];
    //
  case NONLINEAR:
    //
    return _iner_mom[i];
    //
  default:
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "wrong molecular type\n";
    
    throw Error::Logic();
  }
}

double Molecule::imom_sqrt (int i) const 
{
    const char funame [] = "Molecule::imom_sqrt: ";

    switch(type()) {
      //
    case MONOATOMIC:
      //
      std::cerr << funame << "should not be used for atomic fragment\n";

      if(!Structure::mute)
	//
	throw Error::Logic();
      //
    case LINEAR:
      //
      return _imom_sqrt[2];
      //
    case NONLINEAR:
      //
      return _imom_sqrt[i];
      //
    default:
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "wrong molecular type\n";
      
      throw Error::Logic();
    }
}

void Molecule::set_dvd (const D3::Vector& torque, const double* pos, const double* vel, double* pos_drv, double* vel_drv) const 
{
  const char funame [] = "Molecule::set_dvd: ";

  if(type() == MONOATOMIC)
    //
    return;

  double     dtemp;
  D3::Vector vtemp;

  const double* qv;
  double*      qvd;

  switch(type()) {
    //
  case LINEAR:     // laboratory frame is assumed !!!
    //
    // angular vector derivative
    //
    D3::vprod(vel, pos, pos_drv);

    // angular velocity derivative
    //
    for(int i = 0; i < 3; ++i)
      //
      vel_drv[i] = torque[i] / imom(2);

    return;
    //
  case NONLINEAR:  // molecular frame is assumed !!!
    /*
      pos_drv [0] = (- pos[1] * vel[0] - pos[2] * vel[1] - pos[3] * vel[2]) / 2.0;
      pos_drv [1] = (- pos[3] * vel[1] + pos[2] * vel[2] + pos[0] * vel[0]) / 2.0;
      pos_drv [2] = (- pos[1] * vel[2] + pos[3] * vel[0] + pos[0] * vel[1]) / 2.0;
      pos_drv [3] = (- pos[2] * vel[0] + pos[1] * vel[1] + pos[0] * vel[2]) / 2.0;
    */

    // vector part of the quaternion
    //
    qv  = pos     + 1;
    qvd = pos_drv + 1;

    const double& q0  = *pos;
    double&       q0d = *pos_drv;

    // q0d = - qv * omega / 2.
    //
    q0d = -vdot(qv, vel, 3) / 2.;

    // qvd = (qv x omega + q0 * omega) / 2.
    //
    D3::vprod(qv, vel, vtemp); 

    for(int i = 0; i < 3; ++i)
      //
      qvd[i] = (vtemp[i] + q0 * vel[i]) / 2.;

    // rotational frequencies derivatives
    //
    if(top() == SPHERICAL) {
      //
      for(int i = 0; i < 3; ++i)
	//
	vel_drv[i] = torque [i] / imom(i);
    }
    else 
      //
      for(int i = 0; i < 3; ++i) {
	//
	dtemp = (imom((i + 1) % 3) - imom((i + 2) % 3)) / imom(i); // Euler coefficients

	vel_drv[i] = torque [i] / imom(i)  + dtemp * vel[(i + 1) % 3] * vel[(i + 2) % 3]; 
      }
     
    return;
  }
}

// reads molecular geometry from the input stream and uniquely orients the molecular structure to the principal axes 
//
void Molecule::read(std::istream& from) 
{
  const char funame [] = "Molecule::read: ";

  static const double rel_tol = 1.e-5;

  double     dtemp, dmax;
  int        itemp;
  D3::Vector vtemp;
  bool       btemp;

  KeyGroup MoleculeGroup;

  Key geom_key("Geometry");             // molecular geometry
  Key flip_key("FlipAxis");             // 180-degree rotation around flip axis in the standard orientation
  Key char_key("Charge[au]");           // charge
  Key  dip_key("DipoleMoment[au]");     // dipole moment
  Key quad_key("QuadrupoleMoment[au]"); // quadrupole moment
  Key  pol_key("Polarizability[au]");   // dipolar polarizability
  
  std::string token, line, comment, unit, stemp;
  int flip_axis = -1;

  std::vector<double> dipol, quad, polar;

  IO::LineInput lin(from);
  
  // read name
  //
  if(!(lin >> _name)) {
    //
    std::cerr << funame << "cannot read fragment's name\n";

    throw Error::Input();
  }

  while(from >> token) {
    //
    if(token == IO::end_key()) {
      //
      std::getline(from, comment);

      break;
    }
    // geometry section
    //
    else if(token == geom_key) {
      //
      lin.read_line(from);

      if(!(lin >> itemp)) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "could not read number of atoms for the " << _name << " fragment\n";
	
	throw Error::Form();
      }
      
      if(itemp <= 0) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "number of atoms should be positive: " << itemp << "\n";
	
	throw Error::Form();
      }
      resize(itemp);
    
      btemp = false;

      // distance units
      //
      if(lin >> stemp) {
	//
	if(stemp == "angstrom" || stemp == "Angstrom" || stemp == "ang" || stemp == "Ang") {
	  //
	  btemp = true;
	}
	else if(stemp != "bohr" && stemp != "Bohr" && stemp != "au" && stemp != "a.u.") {
	  //
	  if(!Structure::mute)
	    //
	    std::cerr << funame << "unknown distance units: " << stemp << ": available units: angstrom, bohr\n";

	  throw Error::Range();
	}
      }
      else if(!Structure::mute)
	//
	std::cerr << funame << name() << ": WARNING: no distance units: bohr are assumed\n";
    
      // atoms description
      //
      for(iterator at = begin(); at != end(); ++at)
	//
	from >> *at;

      // scale coordinates
      //
      if(btemp)
	//
	for(iterator at = begin(); at != end(); ++at)
	  //
	  *at *= Phys_const::angstrom;
    }
    //  which axes to flip
    //
    else if(token == flip_key) {
      //
      lin.read_line(from);
      
      if(!(lin >> stemp)) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "corrupted\n";
	
	throw Error::Input();
      }

      switch(stemp[0]) {
	//
      case 'X':
	//
	flip_axis = 0;
	
	break;
	//
      case 'Y':
	//
	flip_axis = 1;
	
	break;
	//
      case 'Z':
	//
	flip_axis = 2;
	
	break;
	//
      default:
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << token << ": unknown axis symbol: " << stemp << ": use X, Y, or Z\n";
	
	throw Error::Range();
      }
    }
    // charge
    //
    else if(token == char_key) {
      //
      lin.read_line(from);
      
      if(!(lin >> _charge)) {
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << token << ": input stream is corrupted\n";
	
	throw Error::Input();
      }
    }
    // dipole moment
    //
    else if(token == dip_key) {
      //
      lin.read_line(from);
      
      while(lin >> dtemp)
	//
	_dipole.push_back(dtemp);
    }
    // quadrupole moment
    //
    else if(token == quad_key) {
      //
      lin.read_line(from);
      
      while(lin >> dtemp)
	//
	_quadrupole.push_back(dtemp);
    }
    // polarizability
    //
    else if(token == pol_key) {
      //
      lin.read_line(from);
      
      while(lin >> dtemp)
	//
	_polarizability.push_back(dtemp);
    }
    // unknown key
    //  
    else {
      //
      if(!Structure::mute) {
	//
	std::cerr << funame << "unknown key: " << token << "\nAvailable keys:\n";
	
	Key::show_all(std::cerr);
	std::cerr << "\n";
      }
      throw Error::Form();
    }
  }

  if(!from) {
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "input stream is corrupted\n";
    
    throw Error::Form();
  }

  if(!size()) {
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "no atoms found\n";
    
    throw Error::Logic();
  }

  if(_charge && (_dipole.size() || _quadrupole.size() || _polarizability.size())) {
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "ERROR: with charge no other multipole moments shoud be defined\n";
    
    throw Error::Logic();
  }

  /********************************************************************************
   ************************************ ATOM **************************************
   ********************************************************************************/
  //
  if(size() == 1) {
    //
    _type = MONOATOMIC;
    _top  = SPHERICAL;
    _mass = begin()->mass();
    
    (D3::Vector&)(*begin()) = 0.;

    // multipole moments
    //
    if(_dipole.size() || _quadrupole.size()) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "monoatomic fragment: should have no dipole or quadrupole moments\n";
      
      throw Error::Logic();
    }

    if(_polarizability.size() > 1) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "monoatomic fragment: polarizability should have one (isotropic) component\n";
      
      throw Error::Logic();
    }
    
    return;
    //
  }// atom

  /******************************************************************************
   ***************************** STANDARD ORIENTATION ***************************
   ******************************************************************************/
  //
  // center of mass position
  //
  _std_frame.origin = 0.;
  _mass = 0.;
  //
  for (const_iterator cat = begin(); cat != end(); ++cat) {
    //
    _mass  += cat->mass();
    _std_frame.origin += cat->mass() * (*cat);
  }
  _std_frame.origin /= _mass;

  for (iterator at = begin(); at != end(); ++at)
    //
    *at -= _std_frame.origin;

  // inertia moment matrix
  //
  Lapack::SymmetricMatrix imm (3);
  imm = 0.;
  //
  for (const_iterator cat = begin(); cat != end(); ++cat)
    //
    for (int i = 0; i < 3; ++i)
      //
      for (int j = i; j < 3; ++j)
	//
	imm(i, j) -= cat->mass() * (*cat)[i] * (*cat)[j];

  dtemp = 0.;
  //
  for(int i = 0; i < 3; ++i)
    //
    dtemp -= imm(i, i);

  for(int i = 0; i < 3; ++i)
    //
    imm(i, i) += dtemp;

  // principal inertia moments and vectors
  //
  Lapack::Vector tim(3);
  Lapack::Matrix evec(3);
  tim = imm.eigenvalues(&evec);
  
  // the transformation is a rotation
  //
  if(Lapack::LU(evec).det() < 0.)
    //
    evec.column(1) *= -1.;
    
  // standard orientation matrix
  //
  for (int i = 0; i < 3; ++i)
    //
    _std_frame.orient.row(i) = evec.column(i);

  // manual flip
  //
  if(flip_axis >= 0)
    //
    for(int i = 0; i < 3; ++i)
      //
      if(i != flip_axis)
	//
	_std_frame.orient.row(i) *= -1.;
 
  // symmetry group
  //
  std::string symm_group_name;
  std::vector<Symmetry::SpaceElement> symm_group_base(1, Symmetry::unity());

  /*********************************************************************************
   ******************************* LINEAR FRAGMENT *********************************
   *********************************************************************************/
  //
  if(size() == 2 || tim[0] / tim[2] < tolerance) {
    //
    _type = LINEAR;
    _top = PROLATE;

    // align atoms along x-axis
    //
    for (iterator at = begin(); at != end(); ++at) {
      //
      (*at)[0] = vdot(_std_frame.orient.row(0) ,(const double*)(*at));

      for(int i = 1; i < 3; ++i)
	//
	(*at)[i] = 0.;
    }

    dtemp = std::sqrt(tim[2]);
    //
    for(int i = 0; i < 3; ++i) {
      //
      _iner_mom [i] = tim[2];
      _imom_sqrt[i] = dtemp;
    }

    // multipole moments
    //
    if(_dipole.size() > 1) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "linear fragment: dipole moment should have one component\n";
      
      throw Error::Logic();
    }
    if(_quadrupole.size() > 1) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "linear fragment: quadrupole moment should have one component\n";
      
      throw Error::Logic();
    }
    if(_polarizability.size() > 2) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "linear fragment: polarizability may have at most two components: isotropic and anisotropic\n";
      
      throw Error::Logic();
    }
        
    if(_dipole.size()) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "WARNING: linear fragment: orientation of the symmetry axis  may have flipped; check the sign of the dipole\n";
    }

    /*************************************************************************
     ****************************** SYMMETRY *********************************
     *************************************************************************/
    //
    std::map<double, int> p2i; // position-to-index map

    for(int a = 0; a < size(); ++a)
      //
      p2i[(*this)[a][0]] = a;

    if(p2i.size() != size()) {
      //
      std::cerr << funame << "linear fragment: some atoms have identical coordinates\n";

      throw Error::Range();
    }

    std::map<double, int>::const_iterator         fit = p2i.begin();
    std::map<double, int>::const_reverse_iterator rit = p2i.rbegin();
    
    for(itemp = 0; itemp != size() / 2; ++fit, ++rit, ++itemp)
      //
      if((*this)[fit->second] != (*this)[rit->second] || !are_equal(fit->first, -rit->first, tolerance)) {
	//
	itemp = 0;
	
	break;
      }

    if(itemp && size() % 2 && !are_equal(fit->first, 0., tolerance))
       //
       itemp = 0;
    
    if(itemp) {
      //
      std::cout << "linear fragment has inversion symmetry\n";
      
      symm_group_base.push_back(Symmetry::inversion());
      symm_group_name = "I";
      symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 2));
    }
    else {
      //
      std::cout << "linear fragment has no inversion symmetry\n";
      
      symm_group_name = "E";
      symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 1));
    }
    
    return;
    //
  }// linear molecule

  /***************************************************************************************
   ******************************** NONLINEAR FRAGMENT ***********************************
   ***************************************************************************************/
  //
  _type = NONLINEAR;

  std::set<Permutation> full_perm_group  = ::permutation_symmetry_group(*this, tolerance);

  // find reference atoms
  //
  ref_group.resize(2, -1);
  dmax = 0;
  
  for(const_iterator a = begin(); a != end(); ++a)
    //
    for(const_iterator b = begin(); b !=     a; ++b) {
      //
      dtemp = vdot(D3::vprod(*a, *b));
      
      if(dtemp > dmax) {
	//
	dmax = dtemp;
	
	ref_group[0] = a - begin();
	ref_group[1] = a - begin();
      }
    }
  
  if(std::sqrt(dmax) < tolerance) {
    //
    if(!Structure::mute)
      //
      std::cerr << funame << "cannot find non-colinear atoms\n";
    
    throw Error::Logic();
  }

  /****************************************************************************************
   ************************************* SPHERICAL TOP ************************************
   ****************************************************************************************/
  //
  if((tim[2] - tim[0]) / tim[2] < tolerance) {
    
    _top = SPHERICAL;

    for(int i = 0; i < 3; ++i) {
      //
      _iner_mom  [i] = tim[2];
      _imom_sqrt [i] = std::sqrt(tim[2]);
    }

    // multipole moments
    //
    if(_dipole.size() || _quadrupole.size()) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "spherical top: should be no dipole or quadrupole moments\n";
      
      throw Error::Logic();
    }

    if(_polarizability.size() > 1) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "spherical top: polarizability may have at most one (isotropic) component\n";
      
      throw Error::Logic();
    }

    ref_orient = D3::Matrix((*this)[ref_group[0]], (*this)[ref_group[1]]);

    // symmetry group
    //
    itemp = full_perm_group.size();
    
    if(itemp != 12 && itemp != 24 && itemp != 48 && itemp != 60 && itemp != 120) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "spherical top: unknown symmetry group size: " << itemp << "\n";
      
      throw Error::Logic();
    }

    std::vector<int> max_volume_group(3);

    double max_volume = 0.;
    
    for(int i = 0; i < size(); ++i)
      //
      for(int j = 0; j < i; ++j)
	//
	for(int k = 0; k < j; ++k) {
	  //
	  dtemp = D3::volume((*this)[i], (*this)[j], (*this)[k]);
	  
	  if(dtemp > max_volume) {
	    //
	    max_volume_group[0] = i;
	    max_volume_group[1] = j;
	    max_volume_group[2] = k;
	    max_volume = dtemp;
	  }
	  else if(dtemp < -max_volume) {
	    //
	    max_volume_group[0] = j;
	    max_volume_group[1] = i;
	    max_volume_group[2] = k;
	    max_volume = -dtemp;
	  }
	}

    // no reorientation is necessary
    //
    _std_frame.orient = 1.;

    // symmetry classes
    //
    std::vector<std::set<Permutation> > symm_class;
    
    for(std::set<Permutation>::const_iterator git = full_perm_group.begin(); git != full_perm_group.end(); ++git) {
      //
      btemp = false;
      
      for(std::vector<std::set<Permutation> >::const_iterator sit = symm_class.begin(); sit != symm_class.end(); ++sit)
	//
	if(sit->find(*git) != sit->end()) {
	  //
	  btemp = true;
	  
	  break;
	}
      if(btemp)
	//
	continue;

      symm_class.push_back(std::set<Permutation>());
      
      for(std::set<Permutation>::const_iterator it = full_perm_group.begin(); it != full_perm_group.end(); ++it)
	//
	symm_class.back().insert((*it) * (*git) * it->invert());      
    }

    std::cerr << "there are " << symm_class.size() << " classes:";
    
    for(int i = 0; i < symm_class.size(); ++i)
      //
      std::cerr << "   " << symm_class[i].size();

    return;
    //
  }// spherical top

  /**************************************************************************************
   ************************************ SYMMETRIC TOP ***********************************
   **************************************************************************************/
  //
  // find symmetry axis if any
  //
  int symm_axis = -1;
  
  if((tim[1] - tim[0]) / tim[1] < tolerance) {
    //
    symm_axis = 2;
  }
  else if((tim[2] - tim[1]) / tim[1] < tolerance)
    //
    symm_axis = 0;

  if(symm_axis >= 0) {
    //
    // molecule type
    //
    if(!symm_axis) {
      //
      _top = PROLATE;
    }
    else
      _top = OBLATE;

    D3::Vector y_axis;
    
    for (const_iterator at = begin(); at != end(); ++at) {
      //
      D3::vprod(&_std_frame.orient(symm_axis, 0), *at, y_axis);
      
      dtemp = normalize(y_axis);
      
      if(at == begin() || dtemp > dmax) {
	//
	dmax = dtemp;
	
	_std_frame.orient.row(1) = y_axis;
      }
    }

    if(dmax <  tolerance) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "could not find an atom off the symmetry axis\n";
      
      throw Error::Run();
    }

    if(!symm_axis) {
      //
      D3::vprod(&_std_frame.orient(0, 0), &_std_frame.orient(1, 0), &_std_frame.orient(2, 0));
    }
    else
      //
      D3::vprod(&_std_frame.orient(1, 0), &_std_frame.orient(2, 0), &_std_frame.orient(0, 0));
      

    _iner_mom[symm_axis] = tim[symm_axis];
    _iner_mom[1] = _iner_mom[2 - symm_axis] = tim[1];

    // multipole moments
    //
    if(_dipole.size() > 1) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "symmetric top: dipole moment should have one component\n";
      
      throw Error::Logic();
    }
    if(_quadrupole.size() > 1){
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "symmetric top: quadrupole moment should have one component\n";
      
      throw Error::Logic();
    }
    if(_polarizability.size() > 2) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "symmetric top: polarizability may have at most two components\n";
      
      throw Error::Logic();
    }

    if(_dipole.size()) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "WARNING: symmetric top: orientation of the symmetry axis may have flipped: check the sign of the dipole\n";
    }//
    //
  }// symmetric top

  /************************************************************************************
   ******************************** ASYMMETRIC TOP ************************************
   ************************************************************************************/
  //
  else {
    //
    _top = ASYM;

    for (int i = 0; i < 3; ++i)
      //
      _iner_mom[i] = tim[i];

    // multipole moments
    //
    switch(_dipole.size()) {
      //
    case 0:
      //
      break;
      //
    case 3:
      //
      break;
      //
    default:
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "asymmetric top: dipole moment should have three components: dx, dy, dz\n";
      
      throw Error::Logic();
    }

    switch(_quadrupole.size()) {
      //
    case 0:
      //
      break;
      //
    case 5:
      //
      _quadrupole.push_back(- _quadrupole[0] - _quadrupole[3]);
      
      break;
      //
    default:
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "asymmetric top: quadrupole moment should have five components: qxx, qxy, qxz, qyy, qyz\n";
      
      throw Error::Range();
    }

    switch(_polarizability.size()) {
      //
    case 0:
      //
      break;
      //
    case 1:
      //
      break;
      //
    case 6:
      //
      break;
      //
    default:
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "asymmetric top: polarizability may have either six (pxx, pxy, pxz, pyy, pyz, pzz) or one (isotropic) component\n";
      
      throw Error::Range();
    }

    // reoriented multipole moments

    // dipole
    //
    if(_dipole.size() == 3) {
      //
      D3::Vector dv;
      
      for(int i = 0; i < 3; ++i)
	//
	dv[i] = _dipole[i];

      _dipol_vec = _std_frame.orient * dv;

      for(int i = 0; i < 3; ++i)
	//
	_dipole[i] = _dipol_vec[i];
    }

    // quadrupole
    //
    if(_quadrupole.size()== 6) {
      //
      D3::Matrix mm;
      
      itemp = 0;
      //
      for(int i = 0; i < 3; ++i)
	//
	for(int j = i; j < 3; ++j) {
	  //
	  mm(i, j) = _quadrupole[itemp++];
	  //
	  if(i != j)
	    //
	    mm(j, i) = mm(i, j);
	}
      
      _quad_mat = _std_frame.orient * mm * _std_frame.orient.transpose();

      itemp = 0;
      //
      for(int i = 0; i < 3; ++i)
	//
	for(int j = i; j < 3; ++j)
	  //
	  _quadrupole[itemp++] = _quad_mat(i, j);
    }

    // polarizability
    //
    if(_polarizability.size() == 6) {
      //
      D3::Matrix mm;
      
      itemp = 0;
      
      for(int i = 0; i < 3; ++i)
	//
	for(int j = i; j < 3; ++j) {
	  //
	  mm(i, j) = _polarizability[itemp++];
	  //
	  if(i != j)
	    //
	    mm(j, i) = mm(i, j);
	}
      _polar_mat = _std_frame.orient * mm * _std_frame.orient.transpose();

      itemp = 0;
      
      for(int i = 0; i < 3; ++i)
	//
	for(int j = i; j < 3; ++j)
	  //
	  _polarizability[itemp++] = _polar_mat(i, j);
    }//
    //
  }// asymmetric top

  // atom coordinates in the molecular frame
  //
  for(iterator at = begin(); at != end(); ++at)
    //
    (D3::Vector&)(*at) = _std_frame.orient * (*at);

  ref_orient = D3::Matrix((*this)[ref_group[0]], (*this)[ref_group[1]]);

  for (int i = 0; i < 3; ++i)
    //
    _imom_sqrt[i] = std::sqrt(_iner_mom[i]);

  /************************************* SYMMETRY GROUP *************************************/ 

  // is molecule plane
  //
  bool is_plain = true;
  
  for(const_iterator at = begin(); at != end(); ++at) {
    //
    dtemp = (*at)[2];
    
    dtemp = dtemp < 0. ? -dtemp : dtemp;
    
    if(dtemp > tolerance) {
      //
      is_plain = false;
      
      break;
    }
  }

  // symmetric top
  //
  if(symm_axis >= 0) {
    //
    // plane molecule
    //
    if(is_plain) {

      if(symm_axis != 2) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: symmetry axis should be Z: " << symm_axis << "\n";
	
	throw Error::Logic();
      }

      if(!is_symmetric(Symmetry::reflection(symm_axis))) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: no h-plane symmetry\n";
	
	throw Error::Logic();
      }

      // add the h-plane symmetry element
      //
      symm_group_base.push_back(Symmetry::reflection(symm_axis));

      // major rotation order
      //
      int rotation_order = 0;
      
      for(std::set<Permutation>::const_iterator pit = full_perm_group.begin(); pit != full_perm_group.end(); ++pit) {
	//
	itemp = pit->orbit_max().size();
	
	if(pit == full_perm_group.begin() || itemp > rotation_order)
	  //
	  rotation_order = itemp;
      }
      
      if(rotation_order < 3) {
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "plain symmetric top: for symmetric top major rotation order should be >= 3\n";
	
	throw Error::Logic();
      }
      
      if(full_perm_group.size() % rotation_order) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: rotation order should divide the permutation group size in whole\n";
	
	throw Error::Logic();
      }

      std::ostringstream oss;
      
      oss << rotation_order;
      
      std::string rotation_order_string = oss.str();

      // major_rotation
      //
      Symmetry::SpaceElement major_rotation = Symmetry::rotation(symm_axis, 2. * M_PI / (double)rotation_order);

      // check that the major rotation is the symmetry element
      //
      Permutation rot_perm = is_symmetric(major_rotation);
      
      if(!rot_perm) {
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "plain symmetric top: " << rotation_order << "-fold rotation is not a symmetry element\n";
	
	throw Error::Logic();
      }

      // add major rotation element to the symmetry group base
      //
      symm_group_base.push_back(major_rotation);

      // remove major rotations from symmetry permutations
      //
      Permutation perm(size());
      
      for(int r = 0; r < rotation_order; ++r, perm = rot_perm * perm) {
	//
	std::set<Permutation>::iterator pit = full_perm_group.find(perm);
	
	if(pit == full_perm_group.end()) {
	  //
	  if(!Structure::mute)
	    //
	    std::cerr << funame << "plain symmetric top: rotation element not found\n";
	  
	  throw Error::Logic();
	}
	else
	  //
	  full_perm_group.erase(pit);
      }

      // no in-plane rotations / reflections
      //
      if(!full_perm_group.size()) {
	//
	// symmetry group is C_nh
	//
	symm_group_name = "C" + rotation_order_string + "h";
	
	symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 2 * rotation_order));

	return;
      }

      // there are in-plane rotations / v-reflections
      //
      if(full_perm_group.size() != rotation_order) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: symmetry group unknown\n";
	
	throw Error::Logic();
      }

      // find a pair of atoms coupled by v-reflection
      //
      std::vector<int> orbit = full_perm_group.begin()->orbit_max();
      
      if(orbit.size() != 2) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: not v-reflection\n";
	
	throw Error::Logic();
      }
      
      D3::Vector n = (*this)[orbit[1]] - (*this)[orbit[0]];
      
      n[symm_axis]  =  0.;
      
      normalize(n);
      
      // check that v-reflection is a symmetry element
      //
      if(*full_perm_group.begin() != is_symmetric(Symmetry::reflection(n))) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "plain symmetric top: v-reflection is not a symmetry element\n";
	
	throw Error::Logic();
      }
      
      // add v-reflection to the symmetry elements
      //
      symm_group_base.push_back(Symmetry::reflection(n));

      // symmetry group is D_nh
      //
      symm_group_name = "D" + rotation_order_string + "h";
      
      symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 4 * rotation_order));

      return;
      //
    }// plain molecule

    // non-plain molecule

    // find atoms not in plane
    //
    dmax = 0.;
    std::vector<int> frame_group(3);
    
    for(const_iterator a = begin(); a != end(); ++a)
      //
      for(const_iterator b = begin(); b   !=   a; ++b)
	//
	for(const_iterator c = begin(); c   !=   b; ++c) {
	  //
	  dtemp = D3::volume(*a, *b, *c);
	  
	  if(dtemp > dmax) {
	    //
	    frame_group[0] = a - begin();
	    frame_group[1] = b - begin();
	    frame_group[2] = c - begin();
	    dmax  = dtemp;
	  }
	  else if(dtemp < -dmax) {
	    //
	    frame_group[1] = a - begin();
	    frame_group[0] = b - begin();
	    frame_group[2] = c - begin();
	    dmax  = -dtemp;
	  }
	}

    if(dmax < tolerance) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "non-plain symmetric top: cannot find out-of-plane atoms\n";
      
      throw Error::Logic();
    }

    // extract rotations into separate permutation subgroup
    //
    std::set<Permutation> rot_perm_group;
    
    for(std::set<Permutation>::const_iterator pit = full_perm_group.begin(); pit != full_perm_group.end(); ++pit)
      //
      if(D3::volume((*this)[(*pit)[frame_group[0]]],(*this)[(*pit)[frame_group[1]]],(*this)[(*pit)[frame_group[2]]]) > 0.)
	//
	rot_perm_group.insert(*pit);
    
    for(std::set<Permutation>::const_iterator pit = rot_perm_group.begin(); pit != rot_perm_group.end(); ++pit)
      //
      full_perm_group.erase(*pit);
    
    if(full_perm_group.size() && full_perm_group.size() != rot_perm_group.size()) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: numbers of symmetry elements with and without inversion mismatch\n";
      
      throw Error::Logic();
    }

    // major rotation order
    //
    int rotation_order = 0;
    
    for(std::set<Permutation>::const_iterator pit = rot_perm_group.begin(); pit != rot_perm_group.end(); ++pit) {
      //
      itemp = pit->orbit_max().size();
      
      if(pit == full_perm_group.begin() || itemp > rotation_order)
	//
	rotation_order = itemp;
    }
      
    if(rotation_order < 3) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: major rotation order should be >= 3\n";
      
      throw Error::Logic();
    }
      
    if(rot_perm_group.size() % rotation_order) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: rotation order should divide the rotational subgroup size in whole\n";
      
      throw Error::Logic();
    }

    std::ostringstream oss;
    oss << rotation_order;
    std::string rotation_order_string = oss.str();

    // major_rotation
    //
    Symmetry::SpaceElement major_rotation = Symmetry::rotation(symm_axis, 2. * M_PI / (double)rotation_order);
    
    // check that the major rotation is the symmetry element
    //
    Permutation rot_perm = is_symmetric(major_rotation);
    
    if(!rot_perm) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: " << rotation_order << "-fold rotation is not a symmetry element\n";
      
      throw Error::Logic();
    }

    // add the major rotation symmetry element to the symmetry group basis
    //
    symm_group_base.push_back(major_rotation);

    // remove major rotation elements from the group of rotations
    //
    Permutation perm(size());
    
    for(int r = 0; r < rotation_order; ++r, perm = rot_perm * perm) {
      //
      std::set<Permutation>::iterator pit = rot_perm_group.find(perm);
      
      if(pit == rot_perm_group.end()) {
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "non-plain symmetric top: rotation element not found\n";
	
	throw Error::Logic();
      }
      else
	//
	rot_perm_group.erase(pit);
    }

    // plane symmetry element
    //
    Permutation plane_perm;
    //
    if(full_perm_group.size())
      //
      plane_perm = is_symmetric(Symmetry::reflection(symm_axis));

    if(plane_perm) {
      //
      // add the symmetry plane reflection element to the symmetry group basis
      //
      symm_group_base.push_back(Symmetry::reflection(symm_axis));

      // remove major mirrored rotations from reflection permutations
      //
      perm = Permutation(size());
      
      for(int r = 0; r < rotation_order; ++r, perm = rot_perm * perm) {
	//
	std::set<Permutation>::iterator pit = full_perm_group.find(perm * plane_perm);
	
	if(pit == full_perm_group.end()) {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "non-plain symmetric top: mirrored rotation element not found\n";
	  
	  throw Error::Logic();
	}
	else
	  //
	  full_perm_group.erase(pit);
      }
    }

    // only major rotations are available (no in-plane rotations)
    //
    if(!rot_perm_group.size()) {
      //
      // either  h-reflection and corresponding mirrored rotations or no reflections at all
      //
      if(!full_perm_group.size()) {
	//
	if(plane_perm) {
	  //
	  itemp =  2 * rotation_order;
	  
	  // symmetry group is C_nh
	  //
	  symm_group_name = "C" + rotation_order_string + "h";
	}
	else {
	  //
	  itemp = rotation_order;
	  
	  // symmetry group is C_n
	  //
	  symm_group_name = "C" + rotation_order_string;
	}
	
	// initialize symmetry group
	//
	symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, itemp));

	return;
      }

      // no plane symmetry but there are v-reflections

      // find a pair of atoms coupled by v-reflection
      //
      std::vector<int> orbit = full_perm_group.begin()->orbit_max();
      //
      if(orbit.size() != 2) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "non-plain symmetric top: not v-reflection\n";
	
	throw Error::Logic();
      }
      
      D3::Vector n = (*this)[orbit[1]] - (*this)[orbit[0]];
      n[symm_axis]  =  0.;
      normalize(n);
      
      // check that v-reflection is a symmetry element
      //
      if(*full_perm_group.begin() != is_symmetric(Symmetry::reflection(n))) {
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "non-plain symmetric top: v-reflection is not a symmetry element\n";
	
	throw Error::Logic();
      }
      
      // add v-reflection to the symmetry elements
      //
      symm_group_base.push_back(Symmetry::reflection(n));
      
      symm_group_name = "C" + rotation_order_string + "v";
      
      // symmetry group is C_nv
      //
      symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 2 * rotation_order));
      
      return;
    }

    // there are in-plane rotations
    //
    if(rot_perm_group.size() != rotation_order) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: symmetry group unknown\n";
      
      throw Error::Logic();
    }

    // find the axis of one of the in-plane rotations
    //
    D3::Vector n;
    dmax = 0.;
    
    std::set<std::vector<int> > orbit = rot_perm_group.begin()->orbit();
    
    for(std::set<std::vector<int> >::const_iterator ot = orbit.begin(); ot != orbit.end(); ++ot)
      //
      switch(ot->size()) {
	//
      case 1:
	//
	vtemp = (*this)[(*ot)[0]];
	dtemp = vlength(vtemp);
	
	if(dtemp > dmax) {
	  //
	  n    = vtemp;
	  dmax = dtemp;
	}

	break;
	//
      case 2:
	//
	vtemp = (*this)[(*ot)[0]] + (*this)[(*ot)[1]];
	dtemp = vlength(vtemp) / 2.;
	
	if(dtemp > dmax) {
	  //
	  n    = vtemp;
	  dmax = dtemp;
	}

	break;
	//
      default:
	//
	if(!Structure::mute)
	  //
	  std::cerr << funame << "non-plain symmetric top: not in-plane rotation\n";
	
	throw Error::Logic();
      }
      
    if(dmax < tolerance) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: in-plane rotation axis not found\n";
      
      throw Error::Logic();
    }

    n[symm_axis] = 0.;
    normalize(n);

    // check that in-plane rotation is a symmetry element
    //
    if(*rot_perm_group.begin() != is_symmetric(Symmetry::rotation(n))) {
      //
      if(!Structure::mute)
	//
	std::cerr << funame << "non-plain symmetric top: in-plane rotation is not a symmetry element\n";
      
      throw Error::Logic();
    }

    // add in-plane rotation to the symmetry group base
    //
    symm_group_base.push_back(Symmetry::rotation(n));

    // h-reflection symmetry or no elements with inversion at all
    //
    if(plane_perm || !full_perm_group.size()) {
      //
      if(plane_perm) {
	//
	// symmetry group is D_nh
	//
	itemp = 4 * rotation_order;
	
	symm_group_name = "D" + rotation_order_string + "h";
      }
      else {
	//
	// symmetry group is D_n
	//
	itemp = 2 * rotation_order;
	
	symm_group_name = "D" + rotation_order_string;
      }
      
      // initialize symmetry group
      //
      symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, itemp));

      return;
    }
  
    // no h-reflection symmetry but there are in-plane rotations, v-reflections and mirrored rotations

    // mirrored rotation to half-angle
    //
    Symmetry::SpaceElement mirr_rot = Symmetry::rotation(symm_axis, M_PI / (double)rotation_order) * Symmetry::reflection(symm_axis);

    // check if the mirrored rotation to half-angle is the symmetry element
    //
    if(!is_symmetric(mirr_rot)) {
      //
      if(!Structure::mute)
	//
      std::cerr << funame << "non-plain symmetric top: mirrored rotation is not a symmetry element\n";
      
      throw Error::Logic();
    }
    
    // add mirrored rotation to symmetry group base
    //
    symm_group_base.push_back(mirr_rot);

    // D_nd symmetry group
    //
    symm_group_name = "D" + rotation_order_string + "d";
    
    symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base, 4 * rotation_order));

    return;
    //
  }// symmetric top

  // asymmetric top
  //
  int ref = 0, rot = 0;

  for(int i = 0; i < 3; ++i) {
    //
    if(is_symmetric(Symmetry::reflection(i))) {
      //
      symm_group_base.push_back(Symmetry::reflection(i));
      
      ++ref;
    }
    
    if(is_symmetric(Symmetry::rotation(i))) {
      //
      symm_group_base.push_back(Symmetry::rotation(i));
      
      ++rot;
    }
  }

  int inv = 0;
  
  if(is_symmetric(Symmetry::inversion())) {
    //
    symm_group_base.push_back(Symmetry::inversion());
    
    ++inv;
  }

  // C_1
  //
  if(ref == 0 && rot == 0 && inv == 0) {
    //
    symm_group_name = "C1";
  }
  // C_s
  //
  else if(ref == 1 && rot == 0 && inv == 0) {
    //
    symm_group_name = "Cs";
  }
  // C_2
  //
  else if(ref == 0 && rot == 1 && inv == 0) {
    //
    symm_group_name = "C2";
  }
  // C_i
  //
  else if(ref == 0 && rot == 0 && inv == 1) {
    //
    symm_group_name = "Ci";
  }
  // C_2v
  //
  else if(ref == 2 && rot == 1 && inv == 0) {
    //
    symm_group_name = "C2v";
  }
  // C_2h
  //
  else if(ref == 1 && rot == 1 && inv == 1) {
    //
    symm_group_name = "C2h";
  }
  // D_2
  //
  else if(ref == 0 && rot == 3 && inv == 0) {
    //
    symm_group_name = "D2";
  }
  // D_2h
  //
  else if(ref == 3 && rot == 3 && inv == 1) {
    //
    symm_group_name = "D2h";
  }
  else {
    //
    if(!Structure::mute)
      //
    std::cerr << funame << "asymmetric top: symmetry group is unknown\n";
    
    throw Error::Logic();
  }

  symmetry_group.init(new Symmetry::SpaceGroup(symm_group_name, symm_group_base));
  //
}// read

bool Molecule::is_equal (std::vector<Atom> mol, D3::Vector& shift, Quaternion& orient) const
{
  int        itemp;
  double     dtemp;
  D3::Vector vtemp;

  if(mol.size() != size())
    return false;

  // molecular displacement
  shift = 0.;
  for(int a = 0; a < size(); ++a)
    shift += mol[a].mass() * mol[a];
  shift /= mass();

  for(int a = 0; a < size(); ++a)
    mol[a] -= shift;

  D3::Matrix mol_orient(mol[ref_group[0]], mol[ref_group[1]]);
  
  mol_orient = ref_orient.transpose() * mol_orient;
  for(int a = 0; a < size(); ++a) {
    mol[a] = mol_orient * mol[a];

    if(!are_equal(mol[a], (*this)[a], tolerance))
      return false;
  }

  orient = (Quaternion)mol_orient;
  return true;  
}

void Molecule::print (std::ostream& to, const std::string& indent) const 
{
  const char funame [] = "Molecule::print: ";

  to << name() << "\n";

  switch(type()) {
    //
  case MONOATOMIC:
    //
    to << indent << "   Monoatomic\n";
    
    break;
    //
  case LINEAR:
    //
    to << indent << "   Linear\n"
      //
       << indent << "   Inertia moment = " << imom(0) / Phys_const::amu << " amu*bohr*bohr\n";
    
    break;
    //
  case NONLINEAR:
    //
    switch(top()) {
      //
    case ASYM:
      //
      to << indent << "   Asymmetric top\n";
      
      break;
      //
    case SPHERICAL:
      //
      to << indent << "   Spherical top\n";
      
      break;
    case PROLATE:
      to << indent << "   Prolate symmetric top\n";
      break;
    case OBLATE:
      to << indent << "   Oblate symmetric top\n";
    }
    
    to << indent << "   Inertia moments = {";
    //
    for(int i = 0; i < 3; ++i) {
      //
      if(i)
	//
	to << ", ";
      
      to << imom(i) / Phys_const::amu; 
    }
    to << "} amu*bohr*bohr\n";
  }

  if(symmetry_group)
    //
    to << indent << "   Symmetry group = " << symmetry_group->name()
      //
       << ", group size = "          << symmetry_group->size() << "\n";

  to << indent << "   Mass = " << mass() / Phys_const::amu << " amu\n";
  
  to << indent << "   Standard orientation geometry:\n";
  
  for(std::vector<Atom>::const_iterator at = begin(); at != end(); ++at)
    //
    to << indent << "      " << *at << "\n";
  
  to << "\n";

  // multipole moments
  //
  if(charge() || dipole().size() || quadrupole().size() || polarizability().size()) {
    //
    to << indent << "   Multipole moments:\n";

    // Charge
    //
    if(charge())
      //
      to << indent << "      Charge = " << charge() << "\n";

    // dipole
    //
    if(dipole().size()) { 
      //
      to << indent
	 << std::setw(15) << "DX"
	 << std::setw(15) << "DY"
	 << std::setw(15) << "DZ"
	 << "\n" << indent;

      switch(dipole().size()) {
	//
      case 1:
	//
	if(type() == LINEAR || type() == NONLINEAR && top() == PROLATE) {
	  //
	  to << std::setw(15) << dipole()[0]
	     << std::setw(15) << 0.
	     << std::setw(15) << 0.
	     << "\n";
	}
	else if(type() == NONLINEAR && top() == OBLATE) {
	  //
	  to << std::setw(15) << 0.
	     << std::setw(15) << 0.
	     << std::setw(15) << dipole()[0]
	     << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      case 3:
	//
	if(type() == NONLINEAR && top() == ASYM) {
	  //
	  for(int i = 0; i < 3; ++i)
	    //
	    to << std::setw(15) << dipole()[i];
	  
	  to << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      default:
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "should not be here\n";
	throw Error::Logic();
      }
      
      to << "\n";
      //
    }// dipole

    // quadrupole
    //
    if(quadrupole().size()) {
      //
      to << indent
	 << std::setw(15) << "QXX"
	 << std::setw(15) << "QXY"
	 << std::setw(15) << "QXZ"
	 << std::setw(15) << "QYY"
	 << std::setw(15) << "QYZ"
	 << std::setw(15) << "QZZ"
	 << "\n" << indent;

      switch(quadrupole().size()) {
	//
      case 1: 
	//
	if(type() == LINEAR || type() == NONLINEAR && top() == PROLATE) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    if(i == 0) {
	      //
	      to << std::setw(15) << quadrupole()[0];
	    }
	    else if(i == 3 || i == 5) {
	      //
	      to << std::setw(15) << -quadrupole()[0] / 2.;
	    }
	    else
	      //
	      to << std::setw(15) << 0.;
	  
	  to << "\n";
	}
	else if(type() == NONLINEAR && top() == OBLATE) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    if(i == 5) {
	      //
	      to << std::setw(15) << quadrupole()[0];
	    }
	    else if(i == 3 || i == 0) {
	      //
	      to << std::setw(15) << -quadrupole()[0] / 2.;
	    }
	    else
	      //
	      to << std::setw(15) << 0.;
	  
	  to << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      case 6:
	//
	if(type() == NONLINEAR && top() == ASYM) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    to << std::setw(15) << quadrupole()[i];
	  
	  to << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      default:
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "should not be here\n";
	throw Error::Logic();
      }
      
      to << "\n";
      //
    }// quadrupole
    
    // polarizability
    //
    if(polarizability().size()) {
      //
      to << indent
	 << std::setw(15) << "PXX"
	 << std::setw(15) << "PXY"
	 << std::setw(15) << "PXZ"
	 << std::setw(15) << "PYY"
	 << std::setw(15) << "PYZ"
	 << std::setw(15) << "PZZ"
	 << "\n" << indent;

      switch(polarizability().size()) {
	//
      case 1:
	//
	for(int i = 0; i < 6; ++i)
	  //
	  if(i == 0 || i == 3 || i == 5) {
	    //
	    to << std::setw(15) << polarizability()[0];
	  }
	  else
	    //
	    to << std::setw(15) << 0.;
	
	to << "\n";
	
	break;
	//
      case 2: 
	//
	if(type() == LINEAR || type() == NONLINEAR && top() == PROLATE) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    if(i == 0) {
	      //
	      to << std::setw(15) << polarizability()[0] + polarizability()[1];
	    }
	    else if(i == 3 || i == 5) {
	      //
	      to << std::setw(15) << polarizability()[0] - polarizability()[1] / 2.;
	    }
	    else
	      //
	      to << std::setw(15) << 0.;
	  
	  to << "\n";
	}
	else if(type() == NONLINEAR && top() == OBLATE) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    if(i == 5) {
	      //
	      to << std::setw(15) << polarizability()[0] + polarizability()[1];
	    }
	    else if(i == 3 || i == 0) {
	      //
	      to << std::setw(15) << polarizability()[0] - polarizability()[1] / 2.;
	    }
	    else
	      //
	      to << std::setw(15) << 0.;
	  
	  to << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      case 6:
	//
	if(type() == NONLINEAR && top() == ASYM) {
	  //
	  for(int i = 0; i < 6; ++i)
	    //
	    to << std::setw(15) << polarizability()[i];
	  
	  to << "\n";
	}
	else {
	  //
	  if(!Structure::mute)
	    //
	  std::cerr << funame << "should not be here\n";
	  throw Error::Logic();
	}
	
	break;
	//
      default:
	//
	if(!Structure::mute)
	  //
	std::cerr << funame << "should not be here\n";
	throw Error::Logic();
      }
      
      to << "\n";
      //
    }// polarizability
    //
  }// multipole moments
}
