...
namespace LongRange {
  ...
  int   symmetric_fragment_size () ;
  sym_t symmetry_type   (int frag) ;  // fragment symmetry
  int   orientational_dimension () ;
  int   eff_dof                 () ; // effective number of degrees of freedom
  ...
  public:
    ...
    double integral (double dist, double* mep =0) const ;
  ...
  inline void StatesNumberDensityWrap::isinit () const 
  {
  ...
  inline double StatesNumberDensityWrap::operator() (const Dynamic::Coordinates& dc, double* mep) const 
  {
    isinit();
    return (*_den)(dc, mep);
  }

  inline double StatesNumberDensityWrap::norm_factor() const 
  {
    isinit();
  ...
  public:
    ...
    KDensity (double e, const std::vector<double>& m, double k) ;
    double operator() (const Dynamic::Coordinates&, double* mep =0) const;
    double norm_factor () const { return _nfac; }
  ...
  double j_shift (double amom, double dist) ;

 // symmetric molecule inner rotation energy shift
  double m_shift (const std::vector<double>& m_proj) ;
  ...
  public:
    ...
    MNumber (double e =0., double j =0., const std::vector<double>& m =
	     std::vector<double>(symmetric_fragment_size(), 0.)) ;
  };

  inline MNumber::MNumber (double e, double j, const std::vector<double>& m) 
  ...
  public:
    ...
    KNumber (double e =0., double j =0., const std::vector<double>& m =
	     std::vector<double>(symmetric_fragment_size(), 0.), double k =0.) ;
  };

  inline KNumber::KNumber (double e, double j, const std::vector<double>& m, double k) 
  ...
  inline KIntegral::KIntegral (KNumber* n, double e, double j, const std::vector<double>& m) 
     : MNumber(e, j, m), _num(n), arg_max(-1.), arg_var(0.) 
  {
    _num->set_ener(e); 
    _num->set_amom(j); 
  ...
  inline MDensityIntegral::MDensityIntegral (double d, double e, double j, const std::vector<double>& m) 
     : MNumber(e, j, m), MDensity(e - m_shift(m) - j_shift(j, d)), _dist(d) {}
  ...
  inline MNumberFix::MNumberFix (double d, double e, double j, const std::vector<double>& m)
     : MDensityIntegral(d, e, j, m), _vmin(minimum_energy(d)) {}
  ...
  inline KDensityIntegral::KDensityIntegral (double d, double e, double j, const std::vector<double>& m, double k)
     : KNumber(e, j, m, k), KDensity(e - m_shift(m) - j_shift(j, d), m, k), _dist(d) {}
  ...
  inline KNumberFix::KNumberFix (double d, double e, double j, const std::vector<double>& m, double k) 
  ...
  inline KNumberOpt::KNumberOpt (double d, double e, double j, const std::vector<double>& m, double k)
     : KDensityIntegral(d, e, j, m, k), Bar(j) {}
  ...
  inline MNumberOpt::MNumberOpt (double d, double e, double j, const std::vector<double>& m)
     : MDensityIntegral(d, e, j, m), Bar(j) {}
