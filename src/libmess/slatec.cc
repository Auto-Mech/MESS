...
void Slatec::AdamSolver::run(double& x, double* y, double xout, void* param, Mode mode) 
{
  ...
  for(int count = 0; count < count_max; ++count) {
    //
    ddeabm_(_deriv, _size, x, y, xout, _info, rel_tol, abs_tol, idid, _rwork, _lrw, _iwork, _liw, 0, param);
	...
	//
      case -1:
	//
	std::cerr << funame << "DDEABM has attempted 500 steps: relaxing the error tolerance by 2 and trying again\n";
	rel_tol *= 2.;
	abs_tol *= 2.;
	...
	//
      case -2:
	//
	std::cerr << funame << "the error tolerances have been increased by DDEABM; trying again\n";
	...
	for(int i = 0; i < _size; ++i)
	  //
	  std::cerr << "component = " << i << ", absolute error tolerance = " << abs_tol[i] << "\n";
	...
	//
      default:
	//
	std::cerr << funame << "integrator failed; error code idid = " << idid << "\n";
	throw Error::Run();
      }
    }
    else {
