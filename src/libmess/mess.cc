...
namespace MasterEquation {
  ...
  double energy_step             () { return _energy_step;            }
...
void MasterEquation::set_ped_pair(const std::vector<std::string>& ped_spec) throw(Error::General)
{
...
void MasterEquation::set_default_partition(const std::vector<std::string>& scheme) throw(Error::General)
{
...
void MasterEquation::Well::_set_kernel (const Model::Well& model) throw(Error::General)
{
  ...
  do {
    ...
    if(model.extension() > 0.) {
      itemp = std::ceil((energy_reference() - model.dissociation_limit + model.extension() * temperature()) / energy_step());
      if(itemp > size()) {
	...
	kernel_bandwidth = energy_transfer_form.size();
      ...
      if(Model::Kernel::flags() & Model::Kernel::UP)// energy transfer up probability functional form predefined
	for(int i = size() - 1; i >= 0; --i) {// energy grid cycle
      ...
      else // energy transfer down probability functional form predefined
	for(int i = 0; i < size(); ++i) {// energy grid cycle
	  ...
	  if(a < 0.) {
	    IO::log << IO::log_offset << model.name() 
		    ...
		    << (energy_reference() - (double)i * energy_step()) / Phys_const::incm
		    << " 1/cm";
	    if(Model::Kernel::flags() & Model::Kernel::NOTRUN || Model::escape_size()) {
...
MasterEquation::Well::Well (const Model::Well& model)
{
  ...
  */
  ...
  if(model.escape()) {
    ...
    for(int i = 0; i < size(); ++i) {
      double ener = energy_reference() - (double)i * energy_step();
...
void MasterEquation::Barrier::truncate (int new_size) throw(Error::General)
{
...
#pragma omp parallel for default(shared) schedule(dynamic)
      ...
      for(int r1 = 0; r1 < well(w).crm_size(); ++r1) {
	for(int r2 = r1; r2 < well(w).crm_size(); ++r2)
	  k_22(r1 + well_shift[w], r2 + well_shift[w]) +=  collision_frequency() * 
...
#pragma omp parallel for default(shared) schedule(dynamic)
	  ...
	  for(int r2 = r1; r2 < well(w).crm_size(); ++r2)
	    erk(r1, r2) =  well(w).crm_radiation_rate(r1, r2)
	      + collision_frequency() * well(w).crm_kernel(r1, r2);
	}
      ...
      rmin = well(w).minimal_relaxation_eigenvalue() * collision_frequency();
      rmax = well(w).maximal_relaxation_eigenvalue() * collision_frequency();
    }
  ...
  if(eval_out.is_open()) {
    ...
    eval_out << std::setw(13) << collision_frequency() / Phys_const::herz
	     << std::setw(13) << min_relax_eval / collision_frequency();
    for(int l = 0; l < Model::well_size(); ++l)
      eval_out << std::setw(13) << chem_eval[l] / collision_frequency() << std::setw(13) << rel_proj[l];
    eval_out << "\n";
  }
  ...
  for(int l = 0; l < Model::well_size(); ++l) {
    IO::log << IO::log_offset
	    << std::setw(5)  << l
	    << std::setw(10) << chem_eval[l] / collision_frequency()
	    << std::setw(10) << rel_proj[l]; 
    for(int w = 0; w < Model::well_size(); ++w)
  ...
  if(chem_size < Model::well_size()) {// reduction of species
    ...
    case PROJECTION:
      ...
      for(int l = 0; l < chem_size; ++l)
	IO::log << std::setw(13) << vtemp[l] / collision_frequency();
      IO::log << "\n";
      // well-to-well rate coefficients
  ...
  throw(Error::General)
{
  ...
  std::vector<KineticBasis> kinetic_basis(well_size_max);
  ...
  {// kinetically active basis
	...
	if(eval[itemp] > collision_frequency() * reduction_threshold)
	    break;
  ...
  itemp = 0;
  for(int e = 0; e < well_size_max; itemp += kinetic_basis[e++].active_size)
    well_shift[e] = itemp;
  ...
  for(int e1 = 0; e1 < well_size_max; ++e1)
    for(int e2 = e1; e2 < well_size_max; ++e2)
	...
	for(int l2 = 0; l2 < kinetic_basis[e2].active_size; ++l2) {
	  ...
	  kin_mat(well_shift[e1] + l1, well_shift[e2] + l2) +=  collision_frequency() * dtemp;
	}
  ...
  throw(Error::General)
{
  ...
  // checking
  {
	...
	dtemp = km.eigenvalues()[0] / collision_frequency();
	...
	if(well_partition[e][g].size() > 1) {// bound groups
	  ...
	  dtemp = km.eigenvalues()[1] / collision_frequency();
  ...
  {// kinetic matrix setting
	...
	for(int e2 = e1 + 1; e2 < well_size_max; ++e2)
	  for(int g2 = 0; g2 < well_partition[e2].size(); ++g2) {
		...
		dtemp += well(*w).kernel(e1, e2) * well(*w).state_density(e1);
	    ...
	    kin_mat(i1, i2) = dtemp * collision_frequency();
	  }
	...
	for(Git w = well_partition[e][g].begin(); w != well_partition[e][g].end(); ++w) {
	  dtemp = well(*w).state_density(e);
	  val += well(*w).kernel(e, e) * dtemp;
	  fac += dtemp;
	}      
	const int i = g + well_shift[e];
	kin_mat(i, i) = val / fac * collision_frequency();
      }
  ...
  {// bimolecular product vectors setting
	...
	for(int e1 = 0; e1 < e; ++e1) // only higher energies are considered
	  if(bimolecular_group[e1].size()) {
		...
		if(dtemp != 0.) {
		  btemp = true;
		  vtemp[itemp] -=  dtemp * well(*w).state_density(e) / bg_state_density_sqrt[e1][itemp];
		}
	      }
	     
	    if(btemp) {
	      vtemp = vtemp * bg_mat[e1];
	      vtemp *= 2. * M_PI * collision_frequency();
	      global_bim.row(gi) += vtemp;
	    }
	  }
  ...
  */
  ...
  while(eigenval[itemp] / max_relax_eval < machine_eps) {
    IO::log << IO::log_offset << "WARNING: " << itemp << "-th eigenvalue over collision frequency = " 
  ...
  for(int l = 0; l < Model::well_size(); ++l) {
    IO::log << IO::log_offset
	    << std::setw(5)  << l
	    << std::setw(10) << eigenval[l] / collision_frequency()
	    << std::setw(10) << relaxation_projection[l]; 
    for(int w = 0; w < Model::well_size(); ++w)
  ...
  throw(Error::General)
{
...

  {
	...
	kin_mat(i + well_shift[w], i + well_shift[w]) = cum_stat_num[w][i] / 2. / M_PI
      ...
      if(Model::well(w).escape())
	for(int i = 0; i < well(w).size(); ++i)
	  kin_mat(i + well_shift[w], i + well_shift[w]) += well(w).escape_rate(i);
    }
	...
	for(int j = i; j < well(w).size(); ++j) 
	  kin_mat(i + well_shift[w], j + well_shift[w]) +=  collision_frequency() * well(w).kernel(i, j) 
...
#pragma omp parallel for default(shared) schedule(dynamic)
  ...
  if(Model::inner_barrier_size()) {
    ...
    for(int b = 0; b < Model::inner_barrier_size(); ++b) {
      ...
      IO::log << std::setw(13) << inner_barrier(b).state_number(0) / 2. / M_PI / dtemp / collision_frequency();
    }
    IO::log << "\n";
  }
  // outer barriers
  if(Model::outer_barrier_size()) {
    ...
    for(int b = 0; b < Model::outer_barrier_size(); ++b) {
      int w = Model::outer_connect(b).first;
      IO::log << std::setw(13) 
	      << outer_barrier(b).state_number(0) / 2. / M_PI / well(w).state_density(0) / collision_frequency();
    }
    IO::log << "\n";
  }
	...
	*/
	...
	for(int i = 0; i < well(w).size(); ++i) {
	  ...
	  eigen_global(l, i + well_shift[w]) = chem_evec(w, l) * dtemp / well(w).weight_sqrt()
	    - vdot(&l_21(itemp, l), well(w).crm_row(i), well(w).crm_size(), 1, well(w).size())
	    / dtemp;
	}
    }
  }// low eigenvalue method
  ...
  if(eval_out.is_open()) {
    ...
    for(int l = 0; l < eval_max; ++l)
      eval_out << std::setw(13) << eigenval[l] / collision_frequency() 
  ...
  if(evec_out.is_open()) {
    ...
    for(int l = 0; l < evec_max; ++l) {
      evec_out << "l = " << l << "\n"
	       ...
	       << eigenval[l] / collision_frequency() 
	       << "\n";
...
#pragma omp parallel for default(shared) private(dtemp) schedule(dynamic)
  ...
  for(int i = 0; i < global_size; ++i) {
	...
	dtemp += eigen_global(l, i) * eigen_global(l, j);
      kin_mat(i, j) += dtemp * collision_frequency();
    }
  }
...
void MasterEquation::set_well_partition_method (const std::string& method)  throw(Error::General)
{
...
double MasterEquation::threshold_well_partition (const Lapack::Matrix& pop_chem, Partition& well_partition,
						 Group& bimolecular_group, const std::vector<double>& weight) throw(Error::General)
{
...
void  MasterEquation::high_pressure_analysis () throw(Error::General)
{
...
MasterEquation::PartitionGenerator::PartitionGenerator (int m, int n) throw(Error::General)
...
MasterEquation::GroupGenerator::GroupGenerator (int m, int n) throw(Error::General)
