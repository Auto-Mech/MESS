    ...
    _pot_shift(0.), _external_symmetry(1.), _with_ctf(false)
{
  ...
  std::vector<std::vector<double> > vib_data; // vibrational sampling data

  std::vector<double> pot_data; // potential sampling data
  ...
  while(from >> token) {
	...
	ivec.push_back(itemp);
      ...
      pot_data.resize(_pot_index.size());
	...
	vib_data.resize(_pot_index.size());
      }
	...
	if(_vib_four.size()) {
	  ...
	  while(sampling_input >> dtemp) {
		...
		throw Error::Range();
	      ...
	      vib_data[pl_curr].push_back(dtemp);
	    }	  
	  }

	  // checking
	  //
	  if(vib_data[pl_curr].size() != _vib_four.size()) {
	    //
	    std::cerr << funame << token << ": (";
  ...
  if(force_qfactor && _level_ener_max <= 0.) {
    //
    std::cerr << funame << "quantum state energy limit not initialized\n";
  ...
  std::vector<Lapack::SymmetricMatrix> int_mob_data(_mass_index.size()); // internal mobility sampling data

  std::vector<Lapack::SymmetricMatrix> ext_mob_data;                     // external mobility sampling data

  std::vector<Lapack::Matrix>           coriol_data;                     // coriolis coupling sampling data

  Lapack::Vector                           ctf_data;                     // curvlinear transformation factor sampling data
  ...
  if(1) {
    ...
    if(_with_ctf)
      //
      ctf_data.resize(_mass_index.size());

    // external rotation quantized  
    //
    if(_amom_max) {
      //
      ext_mob_data.resize(_mass_index.size());

      coriol_data.resize(_mass_index.size());
    }
	...
	for(int r = 0; r < internal_size(); ++r) {
      ...
      if(_amom_max)
	//
	coriol_data[g].resize(3, internal_size());
	...
	ctf_data[g] = ctf;
      ...
      if(_amom_max) {
	//
	Lapack::Matrix cc = coriol_data[g] * imm;

	ext_mob_data[g] = inertia_moment_matrix(atom_current).invert();
	
	ext_mob_data[g] /= 2.;

	ext_mob_data[g] += Lapack::SymmetricMatrix(cc * coriol_data[g].transpose());
	...
	for(int r = 0; r < internal_size(); ++r) {
	  //
	  dtemp += double(hv[r] * gv[r]) / double(_mass_index.size(r));
	}

	dtemp *= -2. * M_PI;

	const Lapack::complex cfac = std::exp(Lapack::complex(0., dtemp)) / double(_mass_index.size());
	...
	for(int i = 0; i < internal_size(); ++i) {
	  ...
	  for(int j = i; j < internal_size(); ++j) {
	    ...
	    //#pragma omp atomic
	    //
	    _imm_four[h](i, j) += imm(i, j) * cfac;


	    if(_with_ctf) {
	      //
	      _eff_imm_four[h](i, j) += eff_imm(i, j) * cfac;
	      //
	    }//
	    //
	  }//
	  ...
	  if(_with_ctf) {
	    ...
	    for(int i = 0; i < 3; ++i) {
	      ...
	      for(int j = i; j < 3; ++j) {
		//
		ext_mob_four[h](i, j) += ext_mob(i, j) * cfac;

		for(int l = 0; l < _mass_index.size(); ++l)
	    //
	    vtemp[l] = ext_mob_data[l](i, j);
    ...
    // curvlinear transformation factor initialization and pruning
    //
    if(_with_ctf) {
      //
      ctemp = Lapack::fourier_transform(ctf_data, _mass_index);
  ...
  if(1) {
	...
	// potential
	//
	_pot_four[h] += dfac * pot_data[g];

	// vibrational frequencies
	//
	for(int v = 0; v < _vib_four.size(); ++v) {
	  //
	  _vib_four[v][h] += dfac * vib_data[g][v];
	}//

	if(_with_ctf) {
	  //
	  _eff_pot_four[h] += dfac * pot_data[g] * ctf;

	  // vibrational frequencies
	  //
	  for(int v = 0; v < _vib_four.size(); ++v) {
	    //
	    _eff_vib_four[v][h] += dfac * vib_data[g][v] * ctf;
	  }//
	  //
	}//
	//
	...
	if(_with_ctf) {
	  ...
	  vtemp[g] = pot_data[g] * curvlinear_factor(angle);
	}
	else
	  //
	  vtemp[g] = pot_data[g];
