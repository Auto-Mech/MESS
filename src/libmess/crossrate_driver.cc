...
Dynamic::CCP reactive_region (std::istream& from)
{
  ...
  while(from >> token) {
	...
	throw Error::Init();
      ...
      const DivSur::MultiSur* reg = new DivSur::MultiSur(from);

      //IO::log << IO::log_offset << "SpaceRegion:\n";
      reg->print(IO::log, std::string(IO::log_offset, ' '));
...
Dynamic::CCP stop_condition (std::istream& from)
{
  ...
  while(from >> token) {
	...
	throw Error::Input();
      ...
      set_reactive_energy(dtemp * Phys_const::kcal);
	...
	throw Error::Range();
      ...
      if(res) {
	//
	res |= Dynamic::CCP(new Potential::Condition(pot, reactive_energy()));
      }
      else
	///
	res  = Dynamic::CCP(new Potential::Condition(pot, reactive_energy()));
  ...
  if(mode() == T_MODE) {
    //
    dtemp = reactive_energy() / temperature();

    if(dtemp < -Limits::exp_pow_max()) {
      //
      std::cerr << funame << token << ": reactive energy, " << reactive_energy() / Phys_const::kcal << " kcal/mol, "
      //
		<< " is too low in comparison with the temperature: " << temperature() / Phys_const::kelv << " K\n";
...
int main (int argc, char* argv [])
{
  ...
  if(!IO::log) {
    //
    std::cerr << funame << "cannot open " << stemp << " file\n";
  ...
  while(from >> token) {
    ...
    // crossrate parameters initialization
    //
	...
	throw Error::Init();
      ...
      Dynamic::exclude_region = Dynamic::CCP(new DivSur::MultiSur(from));

      IO::log << IO::log_offset << "exclude region:\n";

      Dynamic::exclude_region->print(IO::log, std::string(IO::log_offset + 3,' '));
  ...
  MultiArray cross_rate(tran, pot, stop);
