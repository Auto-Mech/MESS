...
namespace LongRange {
  ...
  int  euler_angle_index (int frag, ang_t angle) ;
  void ang2dc      (const std::vector<double>&, Dynamic::Coordinates&) ;
...
double LongRange::j_shift (double j, double d) 
{ 
...
double LongRange::m_shift (const std::vector<double>& m) 
{
...
double LongRange::BarrierSearch::_fun (double x) 
{
  double d = std::exp(x);
...
int LongRange::symmetric_fragment_size() 
{
...
LongRange::sym_t LongRange::symmetry_type (int frag) 
{
...
int LongRange::orientational_dimension () 
{
...
void LongRange::init (std::istream& from) 
{
...
LongRange::KDensity::KDensity (double e, const std::vector<double>& m, double k) 
...
int LongRange::euler_angle_index (int frag, LongRange::ang_t angle) 
{
...
void LongRange::ang2dc (const std::vector<double>& angle, Dynamic::Coordinates& dc) 
{
...
double LongRange::StatesNumberDensity::integral (double distance, double* mep) const 
{
...
//
double LongRange::MIntegral::_integral (int frag) const
{
  ...
  if(frag < 0 || frag >  symmetric_fragment_size()) {
    //
    std::cerr << funame << "fragment index out of range: " << frag << "\n";

    throw Error::Range();
  }
  
  if(frag == symmetric_fragment_size()) {
    //
    return (*_num)(); 
  }
...
double LongRange::Opt::_fun (double x) 
{
  _opt_step_count++;
...
double LongRange::Opt::opt_integral () 
{
