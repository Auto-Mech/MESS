...
int main (int argc, char* argv[])
{
  ...
  if(!mpi_rank) {
	...
	break;
      ...
      // send new vertex
      //
      MPI_Send(&vertex, 1, MPI_INT, node, RUN_TAG, MPI_COMM_WORLD);
    ...
    for(int i = node; i < mpi_size; ++i)
      //
      MPI_Send(0, 0, MPI_INT, i, STOP_TAG, MPI_COMM_WORLD);
    ...
    while(node) {
      ...
      // get vertex info
      //
      MPI_Recv(vertex_data, 2, MPI_INT, MPI_ANY_SOURCE, RUN_TAG, MPI_COMM_WORLD, &stat);

      const int mpi_source = stat.MPI_SOURCE;

      // get new energies
      //
      Array<double> vtemp(vertex_data[1]);

      MPI_Recv(vtemp, vertex_data[1], MPI_DOUBLE, mpi_source, RUN_TAG, MPI_COMM_WORLD, &stat);
	...
	++vertex;
     ...
     //
      if(vertex < vertex_size) {
	//
	MPI_Send(&vertex, 1, MPI_INT, mpi_source, RUN_TAG, MPI_COMM_WORLD);

	++vertex;
      }
      else {
	//
	MPI_Send(0, 0, MPI_INT, mpi_source, STOP_TAG, MPI_COMM_WORLD);
   ...
   **********************************************************************************/
    ...
    while(1) {// work loop
      //
      MPI_Recv(vertex_data, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &stat);

      int mpi_tag = stat.MPI_TAG;
	...
	//
	MPI_Send(vertex_data,  2, MPI_INT, 0, RUN_TAG, MPI_COMM_WORLD);

	MPI_Send(ener, ener.size(), MPI_DOUBLE, 0, RUN_TAG, MPI_COMM_WORLD);
	//
      }// run molpro calculation
      //
    }// work loop
